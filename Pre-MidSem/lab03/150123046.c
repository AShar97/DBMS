...
void read_exam_date_time(){
	...
	for (unsigned int i=0; i<count; i++){
		...
		fprintf(fw, "INSERT INTO ett (course_id, exam_date, start_time, end_time) VALUES (\'%s\', \'%s\', \'%s\', \'%s\');\n", course, date, stime, etime);
...
void read_course_stud(char* filename, char course[MAX_STRING]){
	...
	for (unsigned int i=0; i<count; i++){
		...
		fprintf(fw, "INSERT INTO cwsl (cid, serial_number, roll_number, name, email) VALUES (\'%s\', %u, \'%s\', \'%s\', \'%s\');\n", course, number, rollno, name, email);
